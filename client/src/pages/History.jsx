...
const History = ({ darkMode }) => {
  ...
  const fetchHistory = async () => {
    ...
    try {
      ...
      const response = await axios.get(`${API_URL}/api/users/history`, {
      ...
      <div className="container mx-auto max-w-4xl">
        ...
        <div className="space-y-4">
          {historyItems.map((item, idx) => (
            ...
            >
              <div className={`p-2 rounded-lg bg-gradient-to-r ${item.color} text-white mr-3`}>
                {item.icon}
              </div>
              <div className="flex-1">
                ...
                {item.date ? (
                  <p className={`text-xs ${darkMode ? 'text-gray-400' : 'text-gray-500'}`}>
                    <FiCalendar className="inline mr-1 mb-0.5" size={10} />
                    {new Date(item.date).toLocaleString()}
                  </p>
                ) : item.value ? (
                  <p className={`text-lg font-bold ${getScoreColor(history?.atsScore || 0)}`}>
                    {item.value}
                  </p>
                ) : (
        ...
        <motion.div
          ...
          className={`mt-5 p-4 rounded-xl ${darkMode ? 'glass-dark' : 'glass bg-white/50'}`}
        >
          <div className="flex items-center space-x-2 mb-3">
            <FiClock className="text-primary-500 text-sm" />
              ...
              {[...loginHistory].reverse().map((login, idx) => (
                ...
                >
                  <div className="flex items-center space-x-2">
                    <FiLogIn className={darkMode ? 'text-gray-500' : 'text-gray-400'} size={12} />
                    <div>
        ...
        {!history?.createdAt && !history?.atsScore && loginHistory.length === 0 && (
          <motion.div
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
            className={`mt-5 p-8 rounded-xl text-center ${darkMode ? 'glass-dark' : 'glass bg-white/50'}`}
          >
